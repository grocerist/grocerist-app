...
def make_tabulator_data_entry(
    ...
    if name and lng and lat and mentions:
        return {
            ...
            "name": name,
            "geonames": ["geonames", geonames_url],
...
def create_tabulator_data(
    ...
    tabulator_data_output_path = (
        f"{json_file_path.removesuffix('.json')}_tabulator.json"
    )
    tabulator_data = []
    with open(json_file_path, "r") as json_file_io:
        ...
        for row in json_data.values():
            ...
            new_row = make_tabulator_data_entry(
                ...
                alt_names=[
                    row[altnames_key]
                    for altnames_key in altnames_keys
                    if row[altnames_key]
                ],
                total_occurences=row[total_occurences_keys]
                if row[total_occurences_keys]
                else 1,
            )
            if new_row is not None:
                tabulator_data.append(new_row)
    with open(tabulator_data_output_path, "w") as tabulator_data_dumpfile:
        json.dump(tabulator_data, fp=tabulator_data_dumpfile, indent=2)
...
def modify_fields_in_dump(
    json_file_path: str, fieldnames_to_manipulations: dict, write: bool = True
):
    """
    ...
    """
    print(
        f"updating {', '.join(fieldnames_to_manipulations.keys())} in {json_file_path}"
    )
    # load data
    json_data = None
    with open(json_file_path, "r") as json_file_io:
...
def get_play_title_for_mentions(mentions: list):
    ...
    if len(mentions) != 0:
        for mention in mentions:
            new_mentions.append([lookup_play(mention["id"])["title"], mention["value"]])
    return new_mentions
...
if __name__ == "__main__":
    ...
    json_file_paths = br_client.dump_tables_as_json(
        BASEROW_DB_ID, folder_name=JSON_FOLDER, indent=2
    )
    ...
    if os.path.isfile(person_filepath):
        fieldnames_to_manipulations = {"occurences": get_play_title_for_mentions}
        modfied_data = modify_fields_in_dump(
            person_filepath, fieldnames_to_manipulations, False
        )
        with open(person_filepath, "w") as outfile:
            json.dump([val for val in modfied_data.values()], outfile, indent=2)
    if os.path.isfile(places_filepath):
        ...
        modfied_file_path = modify_fields_in_dump(
            places_filepath, fieldnames_to_manipulations
        )
    ...
    if os.path.isfile(vienna_places_filepath):
        test_2_fieldname = {"geonames": lambda x: not bool(x.strip())}
        fieldnames_to_manipulations = {
            "geonames": get_normalized_uri,
            "mentioned_in": get_play_title_for_mentions,
        }
        modfied_file_path = modify_fields_in_dump(
            vienna_places_filepath, fieldnames_to_manipulations
        )
        modfied_file_path = delete_rows_in_dump(modfied_file_path, test_2_fieldname)
        create_tabulator_data(
            json_file_path=modfied_file_path,
            name_key="survey_id",
            lng_key="long",
            ...
            internal_id_key="nestroy_id",
            mentions_key="mentioned_in",
            altnames_keys=["variants", "modern_name"],
            total_occurences_keys="total_occurences",
        )
